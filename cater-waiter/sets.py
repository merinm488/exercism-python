...
def check_drinks(drink_name, drink_ingredients):
    ...
    else: return f'{drink_name} Cocktail'
...
def categorize_dish(dish_name, dish_ingredients):
    ...
    for  item in categories:
        if dish_ingredients.issubset(item[0]):
            return f'{dish_name}: {item[1]}'
...
def compile_ingredients(dishes):
    ...
    full_dish = set()
    for dish in dishes:
        full_dish = full_dish.union(dish)
    return full_dish
...
def singleton_ingredients(dishes, intersection):
    ...
    all_dishes = set()
    for item in dishes:
        all_dishes = all_dishes.union(item)
